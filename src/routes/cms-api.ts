...
cmsApi.get('/forms/:id/submissions', (c) => {
  const id = c.req.param('id')
  const submissions = Array.from(db.submissions.values()).filter(s => s.formId === id)
...
// ============ VERSIONS ============
cmsApi.get('/versions', (c) => {
  const versions = Array.from(db.versions.values())
  return c.json(versions)
})

cmsApi.post('/versions', async (c) => {
  const data = await c.req.json()
